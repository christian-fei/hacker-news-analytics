...
async function main () {
  ...
  browser = await createBrowser({ headless: true, incognito: true })
  server = await createServer({ port: +process.env.PORT || +process.env.HTTP_PORT || 5000 })

  logger.info('creating indexes')
  await createItemsCollectionIndexes()

  await run(queue)
  setInterval(run, 1000 * 60 * 2, queue)
